...
export const setUserCredentials = (userDataService: UserDataService) => {
  ...
  return async (
    ...
    try {
      ...
      const user = await userDataService.one({ userName: tokenResult.username })
      req.context = {
        tokenResult: tokenResult,
        user: user.result
      }
...
export const AuthorizeUserRol = (roles: RolEntityEnum[] = []) => {
  return async (
    ...
    try {
      ...
      const validRroles = context.user && context.user.roles ? context.user.roles.filter((rol: RolEntity) => roles.indexOf(rol.id) !== -1) : []
